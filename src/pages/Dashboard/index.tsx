...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      ...
      // Optimization to avoid format several times during re-render of Component due state change.
      const transactionsAdapted = response.data.transactions.map(
      ...
      const balanceAdapted: Balance = {
        income: formatValue(income),
        outcome: formatValue(outcome),
        total: formatValue(total),
      }

      setTransactions(transactionsAdapted);
      setBalance(balanceAdapted);
